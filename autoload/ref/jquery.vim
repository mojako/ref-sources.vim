...
if !exists('g:ref_jquery_use_cache')
    let g:ref_jquery_use_cache = exists('g:ref_use_cache') ? g:ref_use_cache : 0
endif
"}}}
...
function! s:source.get_keyword()
    ...
    return ref#get_text_on_cursor(
      \ '\%(\$\.\|deferred\.\|event\.\|jQuery\.\|:\|\.\)\?[0-9A-Za-z]\+')
endfunction
...
function s:source.init()
    ...
    if g:ref_jquery_doc_path == ''
        let html = refsrc#get_url('http://jqapi.com/navigation.html')
    else
...
function! s:get_body(query)
    " ページを取得 {{{2
    if g:ref_jquery_doc_path == ''
        let body = refsrc#get_url('http://jqapi.com/' . a:query)
    else
    ...
    let body = substitute(body, '<strong>\s*\(.\{-}\)\s*</strong>', '*\1*', 'g')
